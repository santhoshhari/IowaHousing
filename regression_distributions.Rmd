...
The data dictionary tells us that for most of the fields in \ref{tab:NACount}, `NA` is actually meaningful, indicating non-applicability or a lack of the feature rather than missing data. After checking the data dictionary for the meaning of each field, we updated - for every categorical variable for which `NA` was meaningful - `NA`s with `0`s.
...
Table \ref{tab:NACount1} captures the list of variables with missing data. We impute `NA`s in these variables with
...
From Figure \ref{fig:box1}, we can observe that BrookSide has cheap houses while Northridge and Northridge Heightsare rich neighborhoods with several outliers in terms of price.
